...
export  async function GET(request:Request,{params}:{params:{idGames:string}}){
    ...
    const screenshots=await sql `select screenshoots.screenshoot_url as screenshot_url from screenshoots inner join screenshoots_game on screenshoots_game.screenshoot_id=screenshoots.id where screenshoots_game.game_id=${game[0].id}`
