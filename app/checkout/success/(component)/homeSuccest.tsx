...
export  function HomeSuccess({ orderData }: { orderData: any[] }) {
  ...
  return (
   ...
   // <main className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-gray-950 to-gray-900 text-white p-8">
    ...
    //   <Card className="max-w-3xl w-full bg-gray-900 border border-gray-800 shadow-2xl">
    //     <CardHeader className="flex flex-col items-start gap-2 border-b border-gray-800">
    //       <h2 className="text-xl font-semibold">Detalles de tu orden</h2>
    //       <p className="text-gray-500 text-sm">
    //         Fecha: {new Date(first.created_at).toLocaleDateString()}
    //       </p>
    ...
    //         <div
    //           key={item.id}
    //           className="flex justify-between items-center bg-gray-800 rounded-xl p-4 hover:bg-gray-700 transition-all"
    //         >
    ...
    //               size="sm"
    //               variant="light"
    //               onPress={() => handleCopy(item.code_key)}
    //             >
    ...
    //         <p className="text-green-400 text-xl font-bold">${total.toFixed(2)}</p>
    //       </div>
    //     </CardBody>
    //   </Card>
